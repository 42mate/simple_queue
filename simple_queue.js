...
Drupal.simple_queue = function(base, settings) {
  ...
  var changed = function(item) {
    if (!item.is('.changed')) {
      ...
      $('p.simple_queue-warning').css('color', 'red');
    }
  };
  ...
  var bindButtons = function() {
    ...
    if (settings.clear_list) {
      $(settings.clear_list + ':not(.simple_queue-processed)')
        .addClass('simple_queue-processed')
        .click(function() { return false; })
        .click(function(e) {
    ...
    if (settings.shuffle) {
      $(settings.shuffle + ':not(.simple_queue-processed)')
        .addClass('simple_queue-processed')
        .click(function() { return false; })
        .click(function(e) {
          // randomize the order
    ...
    if (settings.reverse) {
      $(settings.reverse + ':not(.simple_queue-processed)')
        .addClass('simple_queue-processed')
        .click(function() { return false; })
        .click(function(e) {
          // reverse the order
    ...
    if (settings.up) {
      $(settings.up + ':not(.simple_queue-processed)')
        .addClass('simple_queue-processed')
        .click(function() { return false; })
        .click(function(e) {
          var item = $(this).parents(settings.container);
    ...
    if (settings.top) {
      $(settings.top + ':not(.simple_queue-processed)')
        .addClass('simple_queue-processed')
        .click(function() { return false; })
        .click(function(e) {
          var item = $(this).parents(settings.container);
    ...
    if (settings.down) {
      $(settings.down + ':not(.simple_queue-processed)')
        .addClass('simple_queue-processed')
        .click(function() { return false; })
        .click(function(e) {
          var item = $(this).parents(settings.container);
    ...
    if (settings.bottom) {
      $(settings.bottom + ':not(.simple_queue-processed)')
        .addClass('simple_queue-processed')
        .click(function() { return false; })
        .click(function(e) {
          var item = $(this).parents(settings.container);
    ...
    if (settings.add) {
      $(settings.add + ':not(.simple_queue-processed)')
        .addClass('simple_queue-processed')
        .click(function() { return false; })
        .click(function(e) {
          var input = this;
...
Drupal.simple_queue.autoAttach = function() {
  if (Drupal.settings && Drupal.settings.simple_queue) {
    for (var base in Drupal.settings.simple_queue) {
      if (!$('#'+ base + '.simple_queue-processed').size()) {
        var settings = Drupal.settings.simple_queue[base];
        var list = new Drupal.simple_queue(base, settings);
        $('#'+ base).addClass('simple_queue-processed');
      }
    }
  }

  $('a.simple_queue-ajax-toggle').click(function() {
    var a = this;
    href = $(this).attr('href');
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.status) {
          ...
          $(a).html(data.label);
          if (data.qid) {
            $('#simple_queue-count-' + data.qid).html(data.count);
          }
...
if (Drupal.jsEnabled) {
  $(document).ready(Drupal.simple_queue.autoAttach);
}